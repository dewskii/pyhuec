...
class SceneCreateDTO(BaseModel):
    ...
    group: ResourceIdentifierDTO = Field(description="Room or zone this scene belongs to")
    actions: List[ActionDTO] = Field(description="Actions to perform when activating the scene")
    palette: Optional[PaletteDTO] = None
    speed: Optional[float] = Field(None, ge=0.0, le=1.0, description="Speed of dynamic effects")
    auto_dynamic: Optional[bool] = Field(None, description="Enable automatic dynamic palette")
...
class SceneRecallDTO(BaseModel):
    ...
    duration: Optional[int] = Field(None, ge=0, description="Transition duration in milliseconds")
